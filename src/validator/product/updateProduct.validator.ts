...
const updateProductSchema = z.object({
  ...
  value: z.number(
    ...
    }).refine((val: any) => typeof val === 'number' && !isNaN(val) && val >= 0.01 && Number.isInteger(val) === false, {
      message: "O valor do produto deve ser um número maior ou igual a 0.01",
    }).optional(),
  ...
  stock: z.number(
    ...
    }).int().min(0, { message: "O valor do estoque precisa ser no mínimo 0" }).optional(),
});
